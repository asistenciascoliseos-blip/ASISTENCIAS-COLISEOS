...
function updateSummary() {
    ...
    const countLine = state.movement === 'Salida' && (counts.newMembers !== null || counts.oldMembers !== null)
        ? `<br>Socios nuevos: ${counts.newMembers || 0} · Socios antiguos: ${counts.oldMembers || 0}`
        : '';
...
$('take-photo').addEventListener('click', () => {
    ...
    const counts = getExitCounts();
    if (state.movement === 'Salida' && (counts.newMembers !== null || counts.oldMembers !== null)) {
        lines.push(`Nuevos: ${counts.newMembers || 0} · Antiguos: ${counts.oldMembers || 0}`);
    }
