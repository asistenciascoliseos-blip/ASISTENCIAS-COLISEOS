...
function parseUser(raw) {
    try {
        ...
        return {
            id: parsed.id || parsed.usuario || raw,
            name: parsed.name || parsed.nombre || parsed.id || raw,
            area: parsed.area || '',
            sede: parsed.sede || '',
            turno: parsed.turno || '',
            photo: parsed.photo || ''
        ...
        return { id: trimmed, name: trimmed, area: '', sede: '', turno: '', photo: '' };
    }
}
...
$('generate-qr').addEventListener('click', () => {
    ...
    $('qrcode').innerHTML = '';
    new QRCode($('qrcode'), { text: JSON.stringify(profile), width: 200, height: 200, colorDark: '#172033', colorLight: '#ffffff' });
...
function showProfileQr(id) {
    ...
    $('qrcode').innerHTML = '';
    new QRCode($('qrcode'), {
        text: JSON.stringify({ name: profile.name, id, area: profile.area || '', sede: profile.sede || '', turno: profile.turno || '' }),
        width: 200,
        height: 200,
        colorDark: '#172033',
        colorLight: '#ffffff'
