...
$('take-photo').addEventListener('click', () => {
    ...
    const label = `${state.user.name} · ${state.user.id} · ${localDate()} · ${state.movement}`;
    ...
    context.fillText(label, fontSize, canvas.height - barHeight / 2);
...
async function sendAttendance() {
    ...
    try {
        ...
        showToast('No se pudo enviar la asistencia.');
    }
    if (!sent) {
