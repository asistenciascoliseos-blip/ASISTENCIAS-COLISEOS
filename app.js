...
async function sendAttendance() {
    ...
    $('photo-panel').querySelector('.photo-camera').innerHTML = '<div class="success-message">✓<strong>Enviado con éxito</strong><span>Registro exitoso</span></div>';
    setTimeout(() => {
        $('photo-panel').classList.add('hidden');
        $('photo-panel').style.display = 'none';
        $('photo-panel').querySelector('.photo-camera').innerHTML = '<video id="photo-video" autoplay playsinline></video><canvas id="photo-canvas" hidden></canvas><div class="photo-actions"><button id="take-photo" class="primary-button">Tomar foto</button><button id="send-attendance" class="primary-button hidden">Enviar asistencia</button></div>';
        bindPhotoControls();
    }, 1400);
    button.textContent = 'Enviar asistencia';
