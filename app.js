...
function updateSummary() {
    ...
    const countLine = state.movement === 'Salida'
        ? `<br>Socio<br>Socios nuevos<br>Libre<br>Cartilla nueva<br>Cartilla renovada`
        : '';
...
$('import-qr-image').addEventListener('change', async (event) => {
    const file = event.target.files[0];
    ...
    try {
        ...
        if (!user.id || !user.name) throw new Error('Datos incompletos');
        const profiles = getProfiles();
        profiles[user.id] = {
            name: user.name,
            area: user.area || '',
            photo: user.photo || profiles[user.id]?.photo || ''
        };
        localStorage.setItem(profilesKey, JSON.stringify(profiles));
        $('qr-name').value = user.name;
        $('qr-id').value = user.id;
        $('qr-area').value = user.area || '';
        $('generator-message').textContent = 'QR existente guardado correctamente. Ya aparece en la lista.';
        renderProfiles();
    } catch (error) {
        $('generator-message').textContent = 'No se pudo leer ese QR. Usa una imagen clara del código completo.';
    } finally {
