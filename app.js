...
async function startScanner() {
    ...
    try {
        ...
        try {
            ...
            camera = cameras.find((item) => /back|rear|environment|trasera/i.test(item.label)) || cameras[0];
        } catch (error) {
            camera = null;
        }

        const cameraConfig = camera ? camera.id : { facingMode: 'environment' };
        await state.scanner.start(cameraConfig, scanConfig, (decoded) => {
            if (handled) return;
            handled = true;
            selectUser(decoded);
        ...
        $('scanner-message').textContent = 'No se pudo activar la cámara. Revisa el permiso y usa HTTPS.';
        $('start-scanner').disabled = false;
        $('start-scanner').textContent = 'Activar cámara';
    }
}
