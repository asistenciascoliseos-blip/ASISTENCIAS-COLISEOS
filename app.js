...
function requireExitCounts() {
    ...
    $('movement-message').textContent = 'Completa las 4 cantidades de salida antes de escanear.';
    $('movement-message').classList.remove('hidden');
    $('scanner-message').textContent = 'Primero escribe las cantidades de salida.';
    const firstEmpty = ['new-members-count', 'old-members-count', 'new-cards-count', 'renewed-cards-count']
...
function updateSummary() {
    ...
    const countLine = state.movement === 'Salida' && hasExitCounts(counts)
        ? `<br>Socios nuevos: ${counts.newMembers || 0} · Socios antiguos: ${counts.oldMembers || 0}<br>Cartillas nuevas: ${counts.newCards || 0} · Cartillas renovadas: ${counts.renewedCards || 0}`
        : '';
...
$('take-photo').addEventListener('click', () => {
    ...
    if (state.movement === 'Salida' && hasExitCounts(counts)) {
        lines.push(`Socios nuevos: ${counts.newMembers || 0} · Antiguos: ${counts.oldMembers || 0}`);
